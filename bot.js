...
client.on("message", async message => {
 ...
 var prefix= "$";
        ...
        let voicechannels = guild.channels.filter(e => e.type === "voice")
          ...
          .addField(":earth_africa: موقع السيرفر [قريبا]:", `${guild.region}`, true)
...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + "mute")) {
    ...
    if(mention.hasPermission('ADMINISTRATOR')) return message.channel.send(`**لا يمكن آعطاء ميوت لادارة السيرفر**`); \
...
const developers = ["531498806139944962","id"]
client.on('message', message => {
    var argresult = message.content.split(` `).slice(1).join(' ');
      if (!developers.includes(message.author.id)) return;
      
  if (message.content.startsWith(adminprefix + 'setgggg')) {
    client.user.setGame(argresult);
      message.channel.send(`**✅   ${argresult}**`)
  } else 
     if (message.content === (adminprefix + "leaveere")) {
    message.guild.leave();        
  } else  
  if (message.content.startsWith(adminprefix + 'setwsd')) {
  client.user.setActivity(argresult, {type:'WATCHING'});
      message.channel.send(`**✅   ${argresult}**`)
  } else 
  if (message.content.startsWith(adminprefix + 'setlasd')) {
  client.user.setActivity(argresult , {type:'LISTENING'});
      message.channel.send(`**✅   ${argresult}**`)
  } else 
  if (message.content.startsWith(adminprefix + 'setsasd')) {
